...
OUTPUT_PATH = os.path.join(BASE_DIR, "data", "samples", "raw_mixed_eval_data.csv")
...
def _make_anomaly_rows(source_df, provider_context, count, profile, seed):
    ...
    for i in range(count):
        ...
        # Raw-only anomalies still need to be unambiguously abnormal. Put the
        # synthetic claim immediately after the provider's latest historical
        # claim so time-gap and same-day activity features can react.
        ...
        # Add consistent clinical/contextual pressure to rule-generated fraud rows.
        row["has_diabetes"] = 1
        row["has_chf"] = 1
...
def generate_raw_mixed_eval_data(normal_rows=50, anomaly_rows=50, seed=123, output_path=OUTPUT_PATH):
    df = _load_raw_transactions()
    provider_context = _build_provider_context(df)
    if provider_context.empty:
        raise RuntimeError("Not enough provider history in Healthcare_transactions to build raw evaluation data.")
    ...
    print(
        f"Generated {len(mixed_df)} raw-only mixed evaluation rows at {output_path} "
        ...
        "No model scoring was used during generation."
    )
    return mixed_df


def parse_args():
    parser = argparse.ArgumentParser(description="Generate raw-only mixed evaluation data without model filtering.")
